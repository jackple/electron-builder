...
export class NsisUpdater extends BaseUpdater {
  ...
  protected doInstall(options: InstallOptions): boolean {
    ...
    this.spawnLog(options.installerPath, args).catch((e: Error) => {
      ...
      this._logger.info(`Cannot run installer: error code: ${errorCode}, error message: "${e.message}", will be executed again using elevate if EACCES, and will try to use electron.shell.openPath to open installer if ENOENT`)
